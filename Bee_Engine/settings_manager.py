...
class part:
    ...
    def update(self):
        ...
        if self.alive() and self.expand():
            ...
            self.vx = 12#move_x / (float(self.dt) * 1000)
        ...
        # falling down in projectile motion
            ...
            self.cv.move(self.cid, self.vx + move_x, self.vy + GRAVITY * self.dt)
...
def simulate(cv):
    ...
    for point in range(numb_explode):
        ...
        for i in range(1, total_particles):
            r = part(cv, idx=i, total=total_particles, explosion_speed=explosion_speed, x=x_cordi, y=y_cordi,vx=speed, vy=speed, color=color, size=8, lifespan=1.8)
            objects.append(r)
...
if __name__ == '__main__':
    ...
    r = part(cv, idx=0, total=1, explosion_speed=1.2, x=100, y=200, vx=4,
             vy=4, color="green", size=8, lifespan=1.8)
