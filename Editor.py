...
class Editor:
    def __init__(self,file=""):
        Logger.send_info(self, "Setting up the editor", True)
        #file check
        print(str(file))
        self.file = file
        if file == "":
        ...
        if settings.RICH_PRESENCE == True:
            ...
            self.rpc.connect()
            self.rpc.update(state="Using Bee Engine",details=("editing project: "),large_image="bee",start=time.time())
        ###DA REST
        ...
        self.icon = PhotoImage(file='engine/images/bee2.png')
        ...
        #SCRIPT
        self.script_editor_created = False
    ...
    def script_selected(self, event):
        for selected_item in self.script_tree.selection():
            self.item = self.script_tree.item(selected_item)
            self.record = self.item['values']
            # show a message
            #showinfo(title='Information', message=','.join(str(self.record)))
    ...
    def edit_entity(self, entity, Entity):
        ...
        if entity == self.last_entity:
            ...
            ##
            self.save_script_button = customtkinter.CTkButton(master=self.entity_info_frame,text="Save", fg_color="light blue", text_color="white")
            self.save_script_button.place(x=0, y=245)

            self.close_script_button = customtkinter.CTkButton(master=self.entity_info_frame, text="Close Scripts", fg_color="light blue",command=self.hide_editor,text_color="white")
            self.close_script_button.place(x=130, y=245)
            ...
            self.script_tree.bind('<<TreeviewSelect>>', self.script_selected)
